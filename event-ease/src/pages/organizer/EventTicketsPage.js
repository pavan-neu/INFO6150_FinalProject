// src/pages/organizer/EventTicketsPage.js
  ...
  const handleVerifyTicket = async () => {
    ...
    try {
      ...
      if (result.isValid && result.status === "paid") {
        showToast("Ticket is valid and can be used", "success");
      } else if (result.status === "used") {
        showToast("This ticket has already been used", "warning");
      } else if (result.status === "cancelled") {
        showToast("This ticket has been cancelled", "danger");
      } else if (result.status === "reserved") {
        showToast("This ticket is reserved but not paid", "warning");
      }
    } catch (err) {
      console.error("Error verifying ticket:", err);
      showToast("Invalid ticket number or ticket not found", "danger");
      setScanResult({
        isValid: false,
    ...
    <OrganizerLayout>
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4">
        ...
        <div className="mt-3 mt-md-0">
          <Button variant="primary" onClick={() => setShowScanModal(true)}>
            <i className="bi bi-upc-scan me-2"></i> Scan Ticket
          </Button>
        </div>
      </div>
      ...
      <Tab.Container defaultActiveKey="all">
        <Card className="shadow-sm mb-4">
          <Card.Header>
            <Nav variant="tabs">
              <Nav.Item>
                <Nav.Link eventKey="all" onClick={() => setFilterStatus("")}>
                  All Tickets
                </Nav.Link>
              </Nav.Item>
              <Nav.Item>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Scan Ticket</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group className="mb-3">
            ...
            <InputGroup>
              <Form.Control
                type="text"
                placeholder="e.g. EVTEZ-123456-789012"
                value={ticketNumber}
          ...
          {scanResult && (
            ...
            >
              <h5>{scanResult.isValid ? "Valid Ticket" : "Invalid Ticket"}</h5>
              <p className="mb-0">{scanResult.message}</p>
              {scanResult.isValid && scanResult.status === "paid" && (
                <div className="mt-3">
                  <Button
                    variant="success"
              ...
              {scanResult.isValid && scanResult.status === "used" && (
          ...
          <tbody>
            {tickets.map((ticket) => (
              <tr key={ticket._id}>
                <td>
                  <Link
                    to={`/organizer/tickets/${ticket._id}`}
